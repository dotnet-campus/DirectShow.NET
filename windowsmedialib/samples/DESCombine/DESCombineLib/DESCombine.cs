...
namespace DESCombineLib
{
    ...
    public class DESCombine : IDisposable
    {
        ...
        /// <summary>
        /// Configure the graph to output the results to the Windows Media Writer.
        /// </summary>
        /// <param name="sOutputFile">File name for output (must not be null)</param>
        /// <param name="sProfile">String containing the profile.  Must be suitable for sending to
        /// IWMProfileManager.LoadProfileByData.  The Windows Media SDK provides a tool named 
        /// GenProfile.exe which generates files suitable for input to this parameter.  Must not be null or empty.</param>
        /// <param name="pVideoCallback">Callback routine to be called for each video frame or null for no callback</param>
        /// <param name="pAudioCallback">Callback routine to be called for each audio frame or null for no callback</param>
        public void RenderToWM(
            string sOutputFile, 
            string sProfile, 
            IDESCombineCB pVideoCallback, 
            IDESCombineCB pAudioCallback)
        {
            ...
            {
                throw new Exception("Profile cannot be null or blank");
            }
            ...
            try
            {
                ...
                {
                    // Process the profile
                    pProfileManager.LoadProfileByData(sProfile, out pProfile);
                    WMError.ThrowExceptionForHR(hr);

                    try
                    {
                        // Set the output file
                        hr = icgb.SetOutputFileName( MediaSubType.Asf, sOutputFile, out ibfWriter, out pTmpSink);
                        ...
                        try
                        {
                            WindowsMediaLib.IConfigAsfWriter icaw = (WindowsMediaLib.IConfigAsfWriter)ibfWriter;
                            ...
                            // Walk the groups.  For DsConvert, there is one group that 
                            // contains all the video, and a second group for the audio.
                            for (int i = 0; i < NumGroups; i++)
                            {
                                IAMTimelineObj pGroup;
                                ...
                                try
                                {
                                    if (IsVideo(pPin))
                                    {
                                        // Create a sample grabber, add it to the graph and connect it all up
                                        MyCallback mcb = new MyCallback(m_Video, pVideoCallback, (IMediaEventSink) m_pGraph, EC_VideoFileComplete);
                                        RenderHelper(icgb, mcb, "Video", pPin, null, ibfWriter);
                                    }
                                    else
                                    {
                                        // Create a sample grabber, add it to the graph and connect it all up
                                        MyCallback mcb = new MyCallback(m_Audio, pAudioCallback, (IMediaEventSink) m_pGraph, EC_AudioFileComplete);
                                        RenderHelper(icgb, mcb, "Audio", pPin, null, ibfWriter);
                                    }
                                }
                                finally
                                {
                        ...
                        {
                            Marshal.ReleaseComObject(ibfWriter);
                        }
                    }
                    finally
                    {
